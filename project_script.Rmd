...
```{r cache=TRUE}
motorSources <- SCC[grep("motor",SCC$Short.Name,ignore.case = TRUE),c("SCC","Short.Name")]
baltimoreMotorEmissions <- NEI[NEI$SCC %in% motorSources$SCC & NEI$fips == "24510",]
baltimoreToatalMotorEmissions <- with(coalEmissions, tapply(Emissions/1000,year,sum,na.rm=TRUE))
```
...
Emissions from motor vehicle sources in Baltimore City remained almost on the same level up to year 2005 except slight decrease in 2002. However in 2008 situation has changed dramatically when it has shown sharp fall by one third at once. 
